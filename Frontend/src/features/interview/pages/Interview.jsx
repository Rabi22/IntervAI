...
const Interview = () => {
    ...
    if (error || !report || !hasReportContent) {
        return (
            <main className='loading-screen'>
                <h1>{error || "No interview data found for this report."}</h1>
                <p>Please go back and generate a new report or check that the selected interview ID is valid.</p>
                <button className='generate-btn' style={{ marginTop: '1.5rem' }} onClick={() => navigate('/')}>
                    ← Back to Home
                </button>
            </main>
        )
